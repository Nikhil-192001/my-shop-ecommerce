...
if (isset($_POST['register'])) {
	...
	if ($password1 == $password2) {
		...
		if ($email_count == 0) {
			if ($mobile_count == 0) {
				$sql_insert = "insert into `user_register`(`full_name`,`email`,`mobile_number`,`password`)values('$name','$email','$mobile','$password1')";
				mysqli_query($conn, $sql_insert);

				header('location:login_home.php');
			} else { ?>
				<div style="text-align: center; color: red; padding-top:10px;">
					Entered Mobile Number already exists. Please enter a correct mobile number or use Forgot Password option.
				</div>
			<?php }
		} else { ?>
			<div style="text-align: center; color: red; padding-top:10px;">
				Entered Email ID already exists. Please enter a correct email or use Forgot Password option.
			</div>
		<?php }
	} else { ?>
		<div style="text-align: center; color: red; padding-top:10px;">
			Re-entered password does not match. Please enter the same password in both fields.
		</div>
<?php }
}
?>
